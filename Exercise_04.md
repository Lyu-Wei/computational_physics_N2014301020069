...
思路是，一开始只知道曲线的起点（假设为A0），曲线其他部分是未知的，不过通过微分方程,A0的斜率可以被计算出来，也就得到了切线。
顺着切线向前走一小步到点 A1。如果我们假设A1是曲线上的一点（实际上通常不是），那么同样的道理就可以确定下一条切线，依此类推。在经过几步之后，一条折线 A0A1A2A3...就被计算出来了。一般情况下，这条折线与原先的未知曲线偏离不远，并且任意小的误差都可以通过减少步长来得到。
