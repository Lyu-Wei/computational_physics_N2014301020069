...
* 计算
  根据欧拉方法，
  ![](http://latex.codecogs.com/gif.latex?N_%7BA%7D%28t&plus;%5CDelta%20t%29%3DN_%7BA%7D%28t%29&plus;%28%5Cfrac%7BN_%7BB%7D%28t%29%7D%7B%5Ctau%20%7D-%5Cfrac%7BN_%7BA%7D%28t%29%7D%7B%5Ctau%20%7D%29%5CDelta%20t)
  ![](http://latex.codecogs.com/gif.latex?N_%7BB%7D%28t&plus;%5CDelta%20t%29%3DN_%7BB%7D%28t%29&plus;%28%5Cfrac%7BN_%7BA%7D%28t%29%7D%7B%5Ctau%20%7D-%5Cfrac%7BN_%7BB%7D%28t%29%7D%7B%5Ctau%20%7D%29%5CDelta%20t)
  编写程序让计算机计算出所需的所有A和B的N（t）的值，储存在两个列中。
  同时，为了表现N（t）变化的速度，我还让计算机计算了A和B的dN/dt的值，也分别储存在两个列中。
  
* 结果显示
  使用pylab将上一步的得到的四列数据绘制在同一个坐标图中。坐标图如下：
  ![]()
  
* 数值解与解析解的比较
  运算可得problem 1.5 的方程组解析解为
  把数值带入解析解中，将得到的数据与数值解的数据绘制到同一坐标图中进行比较。图像如下（其中虚线为解析解，实线为数值解）：
  ![]()
