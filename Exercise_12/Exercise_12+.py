...
for i in range(101):    
    ...
    for j in range(101):
        if i == 0 or i == 100 or j == 0 or j == 100:
            voltage = 0
        elif 35<=i<=65 and j == 35:
            voltage = 1
        elif 35<=i<=65 and j == 65:
            voltage = -1
        else:
            voltage = 0
...
plt.figure()
CS = plt.contour(X,Y,Z, 8, alpha=.75)
...
strm = ax0.streamplot(X, Y, np.array(Ey), np.array(Ex), linewidth=1)
ax0.set_xlabel('x(m)')
ax0.set_ylabel('y(m)')
ax0.set_xlim(0,100)
ax0.set_title('Electric field near two metal plates')
plt.show()
